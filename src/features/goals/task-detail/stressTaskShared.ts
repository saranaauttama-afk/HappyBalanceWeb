...
export type ParsedStressTaskNote = {
  task: string;
  score: number;
  payload: Record<string, unknown>;
};
...
export function parseStressTaskNote(note: string): ParsedStressTaskNote | null {
  if (!note) return null;

  try {
    const parsed = JSON.parse(note) as Partial<StressTaskNotePayload>;
    if (parsed.entry_type !== "mental_task") return null;
    if (parsed.category !== "mental") return null;
    if (parsed.activity !== "stress-level") return null;
...
export async function syncStressLevelGoal(userId?: string) {
  ...
  const stressLevelGoal = findLatestGoal(
    goalsResponse.data || [],
    (goal) => goal.category === "mental" && goal.activity === "stress-level"
  );

  if (stressLevelGoal) {
    const updateResponse = await goalsService.updateGoal({
      id: stressLevelGoal.id,
    ...
    if (!updateResponse.success) {
      throw new Error(updateResponse.error || "Could not update stress-level goal");
    }
    return;
  }
  ...
  if (!createResponse.success) {
    throw new Error(createResponse.error || "Could not create stress-level goal");
  }
}
