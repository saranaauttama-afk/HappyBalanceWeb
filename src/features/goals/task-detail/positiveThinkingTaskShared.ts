...
type PositiveThinkingNotePayload = {
...
export type ParsedPositiveThinkingNote = {
  task: string;
  score: number;
  payload: Record<string, unknown>;
};
...
export function parsePositiveThinkingTaskNote(note: string): ParsedPositiveThinkingNote | null {
  if (!note) return null;

  try {
    const parsed = JSON.parse(note) as Partial<PositiveThinkingNotePayload>;
    if (parsed.entry_type !== "mental_task") return null;
    if (parsed.category !== "mental") return null;
    if (parsed.activity !== "positive-thinking") return null;
...
export async function syncPositiveThinkingGoal(userId?: string) {
  ...
  const totalTaskCount = POSITIVE_THINKING_TASKS.length;
  ...
  const positiveThinkingGoal = findLatestGoal(
    goalsResponse.data || [],
    (goal) => goal.category === "mental" && goal.activity === "positive-thinking"
  );

  if (positiveThinkingGoal) {
    const updateResponse = await goalsService.updateGoal({
      id: positiveThinkingGoal.id,
    ...
    if (!updateResponse.success) {
      throw new Error(updateResponse.error || "Could not update positive-thinking goal");
    }
    return;
  }
  ...
  if (!createResponse.success) {
    throw new Error(createResponse.error || "Could not create positive-thinking goal");
  }
}
